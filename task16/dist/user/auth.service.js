...
class AuthService {
    ...
    async confirmEmail(bodydata) {
        ...
        const expireConfirmEmail = await this._redisMethods.get(this._redisMethods.confirmEmailKey(email));
        if (!expireConfirmEmail) {
            throw new BadRequestException("verification expired, please signup again");
        }
